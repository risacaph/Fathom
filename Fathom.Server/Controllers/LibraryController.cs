...
using Fathom.Models.DTOs.JumpBar;
using Fathom.Models.DTOs.SignalR;
using Fathom.Models.DTOs.System;
    ...
    private async Task<bool> DeleteLibrary(int libraryId, int userId, CancellationToken ct = default)
    {
        ...
        try
        {
            if (TaskScheduler.HasScanTaskRunningForLibrary(libraryId))
            {
                ...
                throw new FathomException(await localizationService.TranslateAsync(userId, "delete-library-while-scan"));
            }

            var library = await unitOfWork.LibraryRepository.GetLibraryForIdAsync(libraryId, ct: ct);
            if (library == null)
            {
                throw new FathomException(await localizationService.TranslateAsync(userId, "library-doesnt-exist"));
            }

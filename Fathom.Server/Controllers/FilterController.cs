...
using Fathom.Services.Helpers.SmartFilter;
    ...
    : BaseApiController
{
    ...
    private async Task ValidateAndSaveFilterUpsert(string filterName, string encodedFilter,  FilterEntityType entityType)
    {
        ...
        if (string.IsNullOrWhiteSpace(filterName)) throw new FathomException("Name must be set");
        if (Defaults.DefaultStreams.Any(s => s.Name.Equals(filterName, StringComparison.InvariantCultureIgnoreCase)))
        {
            ...
            throw new FathomException("You cannot use the name of a system provided stream");
        }

...
public class ReadingProfileService(IUnitOfWork unitOfWork, ILocalizationService localizationService, IMapper mapper): IReadingProfileService
{
        ...
        UserReadingProfileDto dto, int? activeDeviceId)
    {
        ...
        if (profile == null) throw new FathomException("profile-does-not-exist");
    ...
    public async Task<UserReadingProfileDto> CreateReadingProfile(int userId, UserReadingProfileDto dto)
    {
        ...
        if (await unitOfWork.AppUserReadingProfileRepository.IsProfileNameInUse(userId, dto.Name)) throw new FathomException("name-already-in-use");
    ...
    public async Task<UserReadingProfileDto> PromoteImplicitProfile(int userId, int profileId, int? activeDeviceId)
    {
        ...
        if (profileToPromote == null) throw new FathomException("profile-does-not-exist");
        if (profileToPromote.Kind != ReadingProfileKind.Implicit) throw new FathomException("profile-not-implicit");
        ...
        if (series == null) throw new FathomException("series-doesnt-exist"); // Shouldn't happen
    ...
    public async Task DeleteReadingProfile(int userId, int profileId)
    {
        ...
        if (profile == null) throw new FathomException("profile-doesnt-exist");

        if (profile.Kind == ReadingProfileKind.Default) throw new FathomException("cant-delete-default-profile");
    ...
    public async Task SetSeriesProfiles(int userId, List<int> profileIds, int seriesId)
    {
        ...
        var selectedProfiles = profiles
            ...
            .ToList();
        if (selectedProfiles.Count != profileIds.Count) throw new FathomException("profile-doesnt-exist");
    ...
    public async Task SetProfileDevices(int userId, int profileId, List<int> deviceIds)
    {
        ...
        if (profile.Kind == ReadingProfileKind.Default) throw new FathomException("cant-assign-devices-to-default");
    ...
    private static void DeviceOverlapGuard(List<AppUserReadingProfile> profiles)
    {
        ...
        if (anyOverlap)
        {
            throw new FathomException("reading-profiles-device-overlap");
        }
    }

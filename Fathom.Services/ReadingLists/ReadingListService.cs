...
using Fathom.Models.DTOs.ReadingLists.Request;
...
namespace Fathom.Services.ReadingLists;
    ...
    : IReadingListService
{
    ...
    public async Task<ReadingList> CreateReadingListForUser(AppUser userWithReadingList, string title)
    {
        ...
        if (hasExisting)
        {
            throw new FathomException("reading-list-name-exists");
        }

        var readingList = new ReadingListBuilder(title).Build();
        userWithReadingList.ReadingLists.Add(readingList);

        if (!unitOfWork.HasChanges()) throw new FathomException("generic-reading-list-create");
    ...
    public async Task UpdateReadingList(ReadingList readingList, UpdateReadingListDto dto)
    {
        ...
        if (string.IsNullOrEmpty(dto.Title)) throw new FathomException("reading-list-title-required");

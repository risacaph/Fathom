...
using Fathom.Models.DTOs.KavitaPlus.Account;
...
using Fathom.Models.Entities.Enums.Audit;
...
public class ScrobblingService : IScrobblingService
{
    ...
    private async Task<ScrobbleUpdateContext> CreateScrobbleUpdateContext(int userId, int seriesId, int? chapterId, bool isBackfill, CancellationToken ct)
    {
        ...
        if (series == null) throw new FathomException(await _localizationService.TranslateAsync(userId, "series-doesnt-exist"));

        Chapter? chapter = null;
        if (chapterId.HasValue)
        {
            chapter = await _unitOfWork.ChapterRepository.GetChapterAsync(chapterId.Value, ChapterIncludes.Volumes, ct: ct);
            if (chapter == null) throw new FathomException(await _localizationService.TranslateAsync(userId, "chapter-doesnt-exist"));
        }

        var user = await _unitOfWork.UserRepository.GetUserByIdAsync(userId, AppUserIncludes.UserPreferences, ct);
        if (user == null) throw new FathomException(await _localizationService.TranslateAsync(userId, "user-doesnt-exist"));
    ...
    public async Task ScrobbleReadingUpdateForVolume(int userId, int volumeId, CancellationToken ct = default)
    {
        ...
        if (volume == null) throw new FathomException(await _localizationService.TranslateAsync(userId, "volume-doesnt-exist"));
    ...
    /// <returns></returns>
    /// <exception cref="FathomException">Exceptions may be rethrown as a FathomException</exception>
    ...
    public async Task<int> PostScrobbleUpdate(ScrobbleV3Dto data, string license, ScrobbleEvent evt)
    {
        try
        {
            ...
            if (response.ErrorMessage.Contains("Too Many Requests"))
            {
                ...
                throw new FathomException(RateLimitHitErrorMessage);
            }
            ...
            if (response.ErrorMessage.Contains("Unauthorized"))
            {
                ...
                throw new FathomException("Kavita+ responded with Unauthorized. Please check your subscription");
            }
            ...
            if (response.ErrorMessage.Contains("Access token is invalid"))
            {
                ...
                throw new FathomException("Access token is invalid");
            }
            ...
            throw new FathomException(response.ErrorMessage);
        }
        ...
        #pragma warning restore S2139
        {
            ...
            if (ex.StatusCode == 500 || ex.Message.Contains("Call failed with status code 500 (Internal Server Error)"))
            {
                ...
                throw new FathomException(BadPayLoadErrorMessage);
            }
            throw;
        }
    }

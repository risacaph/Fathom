    ...
    : IStreamService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (user == null) throw new FathomException(await localizationService.TranslateAsync(userId, "no-user"));

        var smartFilter = await unitOfWork.AppUserSmartFilterRepository.GetById(smartFilterId, ct);
        if (smartFilter == null) throw new FathomException(await localizationService.TranslateAsync(userId, "smart-filter-doesnt-exist"));

        var stream = user.DashboardStreams.FirstOrDefault(d => d.SmartFilter?.Id == smartFilterId);
        if (stream != null) throw new FathomException(await localizationService.TranslateAsync(userId, "smart-filter-already-in-use"));
    ...
    public async Task UpdateDashboardStream(int userId, DashboardStreamDto dto, CancellationToken ct = default)
    {
        ...
        if (stream == null) throw new FathomException(await localizationService.TranslateAsync(userId, "dashboard-stream-doesnt-exist"));
        ...
        CancellationToken ct = default)
    {
        ...
        if (stream == null)
        {
            throw new FathomException(await localizationService.TranslateAsync(userId, "dashboard-stream-doesnt-exist"));
        }
        ...
        CancellationToken ct = default)
    {
        ...
        if (externalSource == null) throw new FathomException(await localizationService.TranslateAsync(userId, "external-source-doesnt-exist"));

        var stream = user?.SideNavStreams.FirstOrDefault(d => d.ExternalSourceId == externalSourceId);
        if (stream != null) throw new FathomException(await localizationService.TranslateAsync(userId, "external-source-already-in-use"));
    ...
    public async Task UpdateSideNavStream(int userId, SideNavStreamDto dto, CancellationToken ct = default)
    {
        ...
        if (stream == null)
            throw new FathomException(await localizationService.TranslateAsync(userId, "sidenav-stream-doesnt-exist"));
    ...
    public async Task UpdateSideNavStreamPosition(int userId, UpdateStreamPositionDto dto, CancellationToken ct = default)
    {
        ...
        if (stream == null) throw new FathomException(await localizationService.TranslateAsync(userId, "sidenav-stream-doesnt-exist"));
        ...
        CancellationToken ct = default)
    {
        ...
        if (user == null) throw new FathomException("not-authenticated");

        if (user.ExternalSources.Any(s => s.Host == dto.Host))
        {
            throw new FathomException("external-source-already-exists");
        }

        if (string.IsNullOrEmpty(dto.Name)) throw new FathomException("external-source-required");
        if (!UrlHelper.StartsWithHttpOrHttps(dto.Host)) throw new FathomException("external-source-host-format");
        ...
        CancellationToken ct = default)
    {
        ...
        if (source == null) throw new FathomException("external-source-doesnt-exist");
        if (source.AppUserId != userId) throw new FathomException("external-source-doesnt-exist");

        if (string.IsNullOrEmpty(dto.Host) || string.IsNullOrEmpty(dto.Name)) throw new FathomException("external-source-required");
    ...
    public async Task DeleteSideNavSmartFilterStream(int userId, int sideNavStreamId, CancellationToken ct = default)
    {
        try
        {
            ...
            if (stream == null) throw new FathomException("sidenav-stream-doesnt-exist");

            if (stream.AppUserId != userId) throw new FathomException("sidenav-stream-doesnt-exist");


            if (stream.StreamType != SideNavStreamType.SmartFilter)
            {
                throw new FathomException("sidenav-stream-only-delete-smart-filter");
            }
    ...
    public async Task DeleteDashboardSmartFilterStream(int userId, int dashboardStreamId, CancellationToken ct = default)
    {
        try
        {
            ...
            if (stream == null) throw new FathomException("dashboard-stream-doesnt-exist");

            if (stream.AppUserId != userId) throw new FathomException("dashboard-stream-doesnt-exist");

            if (stream.StreamType != DashboardStreamType.SmartFilter)
            {
                throw new FathomException("dashboard-stream-only-delete-smart-filter");
            }

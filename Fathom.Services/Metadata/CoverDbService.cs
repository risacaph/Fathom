...
using Fathom.Services.Repositories;
using Microsoft.Extensions.Hosting;
using Microsoft.Extensions.Logging;
using NetVips;

namespace Fathom.Services.Metadata;
...
public class CoverDbService : ICoverDbService
{
    ...
    /// </returns>
    /// <exception cref="FathomException">
    ...
    public async Task<string> DownloadFaviconAsync(string url, EncodeFormat encodeFormat, CancellationToken ct = default)
    {
        ...
        if (res.HasValue)
        {
            ...
            throw new FathomException($"Kavita has already tried to fetch from {sanitizedBaseUrl} and failed. Skipping duplicate check");
        }
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(correctSizeLink))
            {
                throw new FathomException($"Could not grab favicon from {baseUrl}");
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            if (res.HasValue)
            {
                ...
                throw new FathomException($"Kavita has already tried to fetch Publisher: {publisherName} and failed. Skipping duplicate check");
            }
            ...
            {
                throw new FathomException($"Could not grab publisher image for {publisherName}");
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                throw new FathomException($"Could not grab person image for {person.Name}");
            }
    ...
    private async Task<string> FallbackToKavitaReaderFavicon(string baseUrl)
    {
        ...
        {
            throw new FathomException($"Could not grab favicon from {baseUrl.Sanitize()}");
        }

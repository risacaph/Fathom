    ...
    : IAnnotationService
{
    ...
    /// <param name="ct"></param>
    /// <returns></returns>
    /// <exception cref="FathomException">Message is not localized</exception>
    public async Task<AnnotationDto> CreateAnnotation(int userId, AnnotationDto dto, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                throw new FathomException("invalid-payload");
            }

            var chapter = await unitOfWork.ChapterRepository.GetChapterAsync(dto.ChapterId, ct: ct) ?? throw new FathomException("chapter-doesnt-exist");
        ...
        catch (Exception ex)
        {
            ...
            throw new FathomException("annotation-failed-create");
        }
    }
    ...
    public async Task<AnnotationDto> UpdateAnnotation(int userId, AnnotationDto dto, CancellationToken ct = default)
    {
        try
        {
            ...
            if (annotation == null || annotation.AppUserId != userId) throw new FathomException("denied");
        ...
        CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new FathomException("annotation-export-failed");
        }
    }

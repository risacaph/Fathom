    ...
    : IThemeService
{
    ...
    {
        var theme = await unitOfWork.SiteThemeRepository.GetThemeDto(themeId) ?? throw new FathomException("theme-doesnt-exist");
        ...
        if (string.IsNullOrEmpty(themeFile) || !directoryService.FileSystem.File.Exists(themeFile))
            throw new FathomException("theme-doesnt-exist");
    ...
    private async Task<string> DownloadSiteTheme(DownloadableSiteThemeDto dto)
    {
        ...
        // {
        //     throw new FathomException("Cannot download theme, hash does not match");
        // }
    ...
    public async Task DeleteTheme(int siteThemeId, CancellationToken ct = default)
    {
        ...
        if (inUse)
        {
            throw new FathomException("errors.delete-theme-in-use");
        }
    ...
    public async Task<SiteTheme> CreateThemeFromFile(string tempFile, string username, CancellationToken ct = default)
    {
        if (!directoryService.FileSystem.File.Exists(tempFile))
        {
            ...
            throw new FathomException("errors.theme-manual-upload");
        }
        ...
        {
            throw new FathomException("errors.theme-already-in-use");
        }
    ...
    /// <param name="ct"></param>
    /// <returns></returns>
    /// <exception cref="FathomException">If theme does not exist</exception>
    public async Task UpdateDefault(int themeId, CancellationToken ct = default)
    {
        try
        {
            ...
            if (theme == null) throw new FathomException("theme-doesnt-exist");

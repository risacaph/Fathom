...
namespace Fathom.Database.Extensions.Filters;
...
public static class ComparisonProfile
{
    ...
    /// <summary>
    /// Throws <see cref="FathomException"/> if the comparison is not in the allowed set.
    /// </summary>
    ...
    public static void Validate(FilterComparison comparison, HashSet<FilterComparison> allowed, string fieldName)
    {
        if (!allowed.Contains(comparison))
            throw new FathomException($"{comparison} is not applicable for {fieldName}");
    }
}
